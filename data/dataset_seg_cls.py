...
class SegDataset(torch.utils.data.Dataset):
    def __init__(self, path, list_path, img_transform = None,target_transform = None,simu_transform = None, mode="train", load_name = False):
    ...
    def __getitem__(self, index):
        # pdb.set_trace()
        img_name = self.list[index].strip("\n")
        label_name  = img_name
        ...
        if self.simu_transform is not None:
            img, seg_label = self.simu_transform(img, [seg_label])

        if self.target_transform is not None:
            seg_label = self.target_transform(seg_label)

        if self.img_transform is not None:
            img = self.img_transform(img)

        if self.load_name:
            return img, seg_label, img_name
        else:
...
class BddLaneClsDataset(torch.utils.data.Dataset):
    def __init__(self, path, list_path, img_transform = None,target_transform = None,simu_transform = None, griding_num=50, load_name = False,
                row_anchor = None,segment_transform=None, num_lanes = 4, mode="train"):
    ...
    def __getitem__(self, index):
        ...
        label_path = os.path.join(self.path, "generated_labels/" + self.mode + "/" + label_name + ".png")
    ...
    # Pyten-20200921-
    # 这里有问题，但该函数运行在resize之前，所以一般 h!=288
    def _get_index(self, label):
        ...
        for i,r in enumerate(sample_tmp):
            ...
            for lane_idx in range(1, self.num_lanes + 1):
                # Pyten-np.where(arr_x,arr_y)，r:row_anchor的位置，图像坐标系第几行，pos为行像素x坐标的平均
